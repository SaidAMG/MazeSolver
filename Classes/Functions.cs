...
namespace WayFinderProto
{
    ...
    {
        internal static void FloorDraw(string[,] _floorplan)
        {
            // uittekenen plattegrond
            for (int x = 0; x < _floorplan.GetLength(0); x++)// Zoekt op X
            {
                for (int y = 0; y < _floorplan.GetLength(1); y++)// Zoekt op X
                {
                    Console.Write(_floorplan[x, y]);// Haalt waarde op met al inde [x,y]
                }
                Console.WriteLine();
            }
        }

        internal static string DescriptionMaker(List<List<int>> _stacklist)
        {
            ...
            for (int step = 1; step < steps.Count() - 1; step++)
            {
                if (steps[step] == steps[step + 1])
                {
                    ...
                    {
                        Description = Description + $"Loop {totalSteps} stappen naar vooruit.\n";
                    }
        ...
        internal static int[] NextStep(int[] _currentPosition, List<List<int>> _visited, int[] _start, int[] _end, string[,] _floorplan)
        {
            ...
            if (NextOption)
            {
                ...
                switch (S)
                {
                    ...
                    case " ":
                        ...
                        if (_visited.Count() > 1)
                        {
                            if (next[0] == _visited[_visited.Count() - 2].ToArray()[0] && next[1] == _visited[_visited.Count() - 2].ToArray()[1])
                            {
                                next[0] = _currentPosition[0] - 1;
                                next[1] = _currentPosition[1];
                            }
                            if (_floorplan[next[0], next[1]] == "x" || _floorplan[next[0], next[1]] == "+" || _floorplan[next[0], next[1]] == "-" || _floorplan[next[0], next[1]] == "|")
                            {
        ...
        internal static string Navigation(int[] _start, int[] _end)
        {
            //Startpunt
            int[] start = _start;
            // Doel
